...
- One file per domain, for example `events.ts` or `participations.ts`
...
- Example: `mapParticipation(row)` returns a view model with derived fields such as medal state

**Composables** (`app/composables/`)

- Wire queries and mappers together using TanStack Query
- Own the loading/error state that components consume
- Named `use<Domain><Action>`, for example `useProvinceProgress` or `useEventList`
- Components should almost never call Supabase directly; always go through a composable

**Views and components** (`app/pages/`, `app/components/`)

- Pages are thin: they pick a layout, call composables, pass data down
- Components receive props and emit events; no direct data fetching
...
app/
  components/       # reusable UI components
    ...
    event/
    participation/
  composables/      # useX hooks; data and behaviour
  constants/        # enums and lookup tables: distances, participation, provinces
  mappers/          # raw rows to view-model transforms
  ...
  utils/            # pure utility functions, for example distance categorisation
i18n/
...
Currently `app/queries/` and `app/mappers/` each contain a single `events.ts` file that covers all domains. Add a new file per domain as the app grows; do not extend the single file indefinitely.
...
`useSupabaseUser()` returns `null` during SSR and briefly on client hydration before the session is restored. Never gate a TanStack Query `enabled` on it, or the query may silently not fire on page load.

Two distinct uses; keep them separate:

- Inside `queryFn` / `mutationFn`: use `supabase.auth.getUser()`. It is async but resolves to the current auth state, making it reliable for the actual data fetch or write.
...
useQuery({
  queryKey: computed(() => ["resource", id, user.value?.id ?? null]),
...
E2E tests live in `test/e2e/`. Auth state is stored in `test/e2e/.auth/user.json`, which is gitignored. Set `E2E_TEST_EMAIL` and `E2E_TEST_PASSWORD` env vars pointing to a dedicated test account before running E2E locally.
...
- Padding inside cards: `p-5` default, `p-6` for large visuals
- Section vertical padding on landing: `py-32` / `pb-24`
- Content max-width: `max-w-3xl` for feature grid, `max-w-sm` for hero copy
...
- Never use `any`; use `unknown` and narrow it. The generated DB types cover most cases, so there is rarely a reason to escape the type system
- Always destructure and check `{ data, error }` from every Supabase call in the query layer; never assume success
- No arbitrary Tailwind values, for example `w-[337px]`; if a custom value is needed more than once, add it to the Tailwind theme
...
- `docs/prompts.md`
- `docs/schema.md` when changing database-related logic
- `docs/setup.md` when setting up a new environment

Private operational details such as provider settings, domains, DNS records, account references, deployment runbooks, rollback notes, and infrastructure maps belong in the private sibling repository `detwaalf-ops`, not in this public app repository. Never store passwords, API keys, tokens, recovery codes, private keys, or service-role keys in git.

## Documentation check

Before finishing work that changes code, configuration, schema, infrastructure, deployment behavior, user-facing flows, or project conventions, explicitly check whether documentation needs to be updated.

In the final response, always state one of:

- `Docs updated:` followed by the files that changed.
- `Docs not needed:` followed by a short reason.

Update public project docs in this repository when the change affects code architecture, setup, schema, product behavior, or contributor guidance. Update private ops docs in the sibling `detwaalf-ops` repository when the change affects providers, domains, DNS, hosting, deployment runbooks, accounts, infrastructure, or operational procedures.
