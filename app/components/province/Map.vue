...
const props = defineProps<{
  completedProvinces: Partial<Record<DistanceCategory, Set<number>>>;
  upcomingProvinces: Partial<Record<DistanceCategory, Set<number>>>;
...
const selectedPath = computed(() => {
  ...
  const entry = Object.entries(SLUG_TO_ID).find(
    ([, id]) => id === props.selectedProvinceId,
  );
...
function toggleProvince(slug: string) {
  ...
  emit(
    ...
    props.selectedProvinceId === id ? null : id,
  );
}
...
function fillColor(slug: string): string {
  ...
  if (activeDistance !== "all") {
    if (completedProvinces[activeDistance]?.has(id))
      return DISTANCE_COLORS[activeDistance];
    if (upcomingProvinces[activeDistance]?.has(id))
      return `url(#stripe-${activeDistance})`;
    return "#e5e7eb";
  }

  if (completedProvinces.marathon?.has(id)) return DISTANCE_COLORS.marathon;
  if (completedProvinces.half?.has(id)) return DISTANCE_COLORS.half;
  if (completedProvinces["10k"]?.has(id)) return DISTANCE_COLORS["10k"];
