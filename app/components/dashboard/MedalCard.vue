    ...
    <div :class="!compact ? 'min-h-14' : ''">
      ...
      <template v-if="provinceSelected">
        <template v-if="eventName">
          ...
          <p
            ...
            :class="
              compact
                ? 'mt-2 text-xs font-medium text-gray-900 leading-tight line-clamp-2'
        ...
        <p
          v-else
          :class="
            compact ? 'mt-2 text-xs text-gray-400' : 'text-sm text-gray-400'
          "
        >
        ...
        <p v-if="eventDate" class="text-xs text-gray-400 mt-0.5">
          {{ eventDate
          }}<template v-if="finishTime"> · {{ finishTime }}</template>
        </p>
      </template>
      ...
      <template v-else>
        <div
          :class="
            compact
              ? 'mt-2 text-xl font-bold text-gray-900 leading-none'
        ...
        <div
          :class="
            compact
              ...
              : 'mt-3 h-1.5 bg-gray-100 rounded-full overflow-hidden'
          "
        >
          <div
