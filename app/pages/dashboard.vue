...
type ParticipationStatus = 'interested' | 'signed_up' | 'completed' | 'dns' | 'dnf'
...
const completedProvinces: Record<Distance, Set<number>> = {
  '10k': new Set([2, 7, 9, 11]),    // fr, ut, zh, nb
  half: new Set([6, 4]),             // ge, ov
  marathon: new Set([8, 12]),        // nh, li
}

const upcomingProvinces: Record<Distance, Set<number>> = {
  '10k': new Set(),
...
const upcomingEvents: UpcomingEvent[] = [
  { name: 'dsm-firmenich Maastricht Marathon', province: 'Limburg', distance: 'Halve marathon', date: '24 mei', status: 'signed_up' },
  { name: 'Marathon Amersfoort', province: 'Utrecht', distance: 'Halve marathon', date: '31 mei', status: 'signed_up' },
  { name: 'Halve Marathon Vlieland', province: 'Friesland', distance: 'Halve marathon', date: '9 aug', status: 'signed_up' },
]
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-5">
      ...
      <div class="bg-white rounded-xl border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-5">
          <span class="text-sm font-medium text-gray-900">{{ t('dashboard.provincesLabel') }}</span>
          <div class="flex items-center gap-1">
            <button
              v-for="filter in filters"
              :key="filter.key"
              class="px-3 py-1 rounded-full text-xs font-medium transition-colors"
              :class="activeDistance === filter.key
      ...
      <div class="flex flex-col gap-4">
        ...
        >
          <div class="flex items-start justify-between mb-2">
            ...
            <span class="text-xs font-medium px-2 py-0.5 rounded-full" :class="track.badgeClass">
              {{ track.medal }}
            </span>
          </div>
          <div class="text-2xl font-bold text-gray-900">
            {{ track.count }}<span class="text-sm font-normal text-gray-400">/12</span>
          </div>
          <div class="mt-3 h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              class="h-full rounded-full transition-all"
              :style="{ width: `${(track.count / 12) * 100}%`, background: track.color }"
            />
          </div>
        </div>
          ...
          <div class="flex flex-col divide-y divide-gray-50">
            ...
            >
              <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">{{ event.province }} · {{ event.distance }}</p>
              </div>
